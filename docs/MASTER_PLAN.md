...
### 10.1 Debug logs leak JWT tokens
...
### 10.2 JWT server-side blocklist on logout
...
### 10.4 HTTPS / encrypted transport to the Pi
...
### 10.5 JWT secret rotation procedure
...
### 10.6 iOS Keychain accessibility level
...
### 10.9 Raw SQL in ReportsController
**Risk:** `ReportsController` uses raw SQL with `#{@current_user.id}` interpolation (lines 9, 18, 29, 40, 65, 85, 106). Currently safe because it's an integer from an authenticated session, not user input. But:
- A future maintainer could interpolate `params[...]` the same way → SQL injection
- Silent breakage if a typo omits the user scope
- History search with `ILIKE` should also be verified to use parameterized bindings
**Fix:** Refactor to use ActiveRecord query methods or parameterized queries (`WHERE user_id = ?` with bindings). Grep `app/controllers/**/*.rb` for `ILIKE` / `LIKE` / string interpolation in SQL.
...
|---|------|------|--------|--------|
| 1 | **Debug logs leak tokens (10.1)** | Security | Tiny | Token visible via Console.app |
| 2 | Test coverage gaps (2.1) | Testing | Small | Catches regressions |
| 3 | Sleep replacement (2.2) | Testing | Small | 30-60s faster runs |
| 4 | Password Manager fix (1.1) | Bug | Small | Eliminates test flakiness |
| 5 | test.sh coverage revert (1.2) | Bug | Tiny | Correctness |
| 6 | Keychain accessibility level (10.6) | Security | Tiny | Restricts token access |
| 7 | Mass-assignment audit (10.7) | Security | Small | Defensive |
| 8 | Raw SQL in ReportsController (10.9) | Security | Small | Defensive refactor |
| 9 | Login rate limiting (10.3) | Security | Small | Brute-force prevention |
| 10 | Settings tab (5.1) | Feature | Medium | User needs this |
| 11 | Session-share 3 test classes (9.4) | Testing | Small | 25 login cycles saved |
| 12 | Additional offline queue tests (9.5) | Testing | Small | Covers race conditions |
| 13 | CSV export (5.5) | Feature | Small | Data portability |
| 14 | CSV upload validation (10.8) | Security | Small | DOS + malformed input |
| 15 | Split FinalDashboardView (3.1) | Refactor | Medium | Maintainability |
| 16 | Spending trends chart (5.4) | Feature | Medium | User insight |
| 17 | Cache staleness / TTL (1.4) | Bug | Medium | Multi-device consistency |
| 18 | History pagination (5.6) | Feature | Medium | Scales for large datasets |
| 19 | JWT blocklist on logout (10.2) | Security | Medium | Token revocation |
| 20 | HTTPS / Tailscale to Pi (10.4) | Security | Medium | Encrypted transport |
| 21 | Recurring expenses (5.2) | Feature | Large | Saves manual work |
| 22 | Offline edits phase 3.2 (4.2) | Feature | Large | Full offline support |
| 23 | Retry logic phase 4 (4.3) | Feature | Medium | Reliability |
| 24 | Budget alerts (5.3) | Feature | Medium | Proactive tracking |
| 25 | Extract loadData pattern (3.2) | Refactor | Medium | Code quality |
| 26 | Rails test suite (2.3) | Testing | Large | Backend safety |
| 27 | CI pipeline (9.1) | Process | Medium | Catches regressions in main |
| 28 | Version/release discipline (9.2) | Process | Small | Change tracking |
| 29 | Production monitoring (9.3) | Process | Medium | Proactive Pi alerts |
| 30 | JWT secret rotation runbook (10.5) | Security | Small | Docs only; needs 10.2 first |
| 31 | Dark mode QA (5.7) | Polish | Small | Visual correctness |
| 32 | Localization (5.8) | Polish | Large | Only needed if sharing app |
